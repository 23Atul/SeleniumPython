...
# for returning date
        r_date = driver.find_element(By.XPATH, "//span[contains(@class,'css-xf2jy5')]")
        r_date.click()
        all_dates = driver.find_elements(
            By.XPATH, "//div[@class= 'react-datepicker__month-container']//div[@class='react-datepicker__month']//div[@class!='react-datepicker__day--disabled ']")
        time.sleep(2)

        for date in all_dates:
            # print(date.get_attribute("aria-label"))
            if date.get_attribute("aria-label") == "Choose Sunday, April 20th, 2025":
